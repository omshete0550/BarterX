...
import { createProductRequest, deleteProductRequest, getMyProductsRequest, getProductRequest, getProductsRequest, updateProductRequest, visualSearchProductsRequest } from "./productAPI";
...
export const normalizeProduct = (product) => ({ ...product, id: product._id, category: label(product.category), condition: label(product.condition), image: product.images?.[0] || "https://placehold.co/800x600/f0ebff/6d3df5?text=BarterX" });
...
export const searchProductsByImage = createAsyncThunk("products/searchByImage", async (image, api) => {
  try { return (await visualSearchProductsRequest(image)).data.data; } catch (error) { return api.rejectWithValue(message(error)); }
});
...
const productSlice = createSlice({
  name: "products",
  initialState: { items: [], myItems: [], currentProduct: null, pagination: null, loading: false, creating: false, error: null, visualItems: [], visualPagination: null, visualLoading: false, visualError: null },
  reducers: {
    clearProductError: (state) => { state.error = null; },
    clearVisualSearch: (state) => { state.visualItems = []; state.visualPagination = null; state.visualError = null; },
  },
  extraReducers: (builder) => builder
    .addCase(fetchProducts.pending, (state) => { state.loading = true; state.error = null; })
    .addCase(fetchProducts.fulfilled, (state, action) => { state.loading = false; state.items = action.payload.products.map(normalizeProduct); state.pagination = action.payload.pagination; })
    .addCase(fetchProducts.rejected, (state, action) => { state.loading = false; state.error = action.payload; })
    .addCase(searchProductsByImage.pending, (state) => { state.visualLoading = true; state.visualError = null; })
    .addCase(searchProductsByImage.fulfilled, (state, action) => { state.visualLoading = false; state.visualItems = action.payload.products.map(normalizeProduct); state.visualPagination = action.payload.pagination; })
    .addCase(searchProductsByImage.rejected, (state, action) => { state.visualLoading = false; state.visualError = action.payload; })
    .addCase(fetchProductById.pending, (state) => { state.loading = true; state.currentProduct = null; state.error = null; })
    .addCase(fetchProductById.fulfilled, (state, action) => { state.loading = false; state.currentProduct = normalizeProduct(action.payload); })
    ...
    .addCase(createProduct.fulfilled, (state, action) => { state.creating = false; state.items.unshift(normalizeProduct(action.payload)); })
    ...
    .addCase(fetchMyProducts.fulfilled, (state, action) => { state.loading = false; state.myItems = action.payload.map(normalizeProduct); })
    ...
    .addCase(updateProduct.fulfilled, (state, action) => { const product = normalizeProduct(action.payload); state.creating = false; state.currentProduct = product; state.myItems = state.myItems.map((item) => item.id === product.id ? product : item); })
...
export const { clearProductError, clearVisualSearch } = productSlice.actions;
